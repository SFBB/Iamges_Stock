...
class database_handle():
	...
	def add_kinds(self, kinds, image_id):
		for kind in kinds:
			...
			self.db.kinds.update_one({"name": kind}, {"$push": {"image_list": image_id}})
		self.db.images.update_one({"_id": image_id}, {"$set": {"last_edit_time": get_date()}})
	...
	def add_labels(self, labels, image_id):
		for label in labels:
			...
			self.db.labels.update_one({"name": label}, {"$push": {"image_list": image_id}})
		self.db.images.update_one({"_id": image_id}, {"$set": {"last_edit_time": get_date()}})
	...
	def add_tags(self, tags, album_id):
		for tag in tags:
			...
			self.db.tags.update_one({"name": tag}, {"$push": {"album_list": album_id}})
		self.db.albums.update_one({"_id": album_id}, {"$set": {"last_edit_time": get_date()}})

	def add_image_to_album(self, image_id, album_id):
		self.db.albums.update_one({"_id": album_id}, {"$push": {"image_list": image_id}})
		self.db.albums.update_one({"_id": album_id}, {"$set": {"last_edit_time": get_date()}})

	def mark_image_as_secret(self, image_ids, status):
		for image_id in image_ids:
			self.db.images.update_one({"_id": image_id}, {"$set": {"secret": status}})
			self.db.images.update_one({"_id": image_id}, {"$set": {"last_edit_time": get_date()}})

	def mark_album_as_secret(self, album_ids, status):
		for album_id in album_ids:
			self.db.albums.update_one({"_id": album_id}, {"$set": {"secret": status}})
			self.db.albums.update_one({"_id": album_id}, {"$set": {"last_edit_time": get_date()}})

	def update_infos_on_image(self, info, image_id):
		for i in info:
			if i == "label":
				origin = self.db.images.find({"_id": image_id})[0]["label"]
			...
			if i == "kind":
				origin = self.db.images.find({"_id": image_id})[0]["kind"]
				for ii in info["kind"]:
					if ii not in origin:
			...
			self.db.images.update_one({"_id": image_id}, {"$set": {i: info[i]}})
		self.db.images.update_one({"_id": image_id}, {"$set": {"last_edit_time": get_date()}})

	def update_infos_on_album(self, info, album_id):
		for i in info:
			if i == "tag":
				origin = self.db.images.find({"_id": album_id})[0]["tag"]
				for ii in info["tag"]:
					if ii not in origin:
			...
			self.db.images.update_one({"_id": album_id}, {"$set": {i: info[i]}})
		self.db.images.update_one({"_id": album_id}, {"$set": {"last_edit_time": get_date()}})
	...
	def remove_image_from_label(self, image_id, label):
		image_list = self.db.labels.find({"name": label})[0]["image_list"]
		if image_id in image_list:
			self.db.labels.update_one({"name": label}, {"$pull": {"image_list": image_id}})
		else:
			return
	...
	def remove_image_from_kind(self, image_id, kind):
		image_list = self.db.kinds.find({"name": kind})[0]["image_list"]
		if image_id in image_list:
			self.db.kinds.update_one({"name": kind}, {"$pull": {"image_list": image_id}})
		else:
			return
	...
	def remove_album_from_tag(self, album_id, tag):
		album_list = self.db.tags.find({"name": tag})[0]["album_list"]
		if album_id in album_list:
			self.db.tags.update_one({"name": tag}, {"$pull": {"album_list": album_id}})
		else:
			return

	def remove_image_from_album(self, image_id, album_id):
		image_list = self.db.albums.find({"_id": album_id})[0]["image_list"]
		if image_id in image_list:
			self.db.albums.update_one({"_id": album_id}, {"$pull": {"image_list": image_id}})
		else:
			return

	def remove_images(self, image_ids):
		for image_id in image_ids:
			image_info = self.db.images.find({"_id": image_id})[0]
			...
			self.db.images.remove({"_id": image_id})

	def remove_albums(self, album_ids):
		for album_id in album_ids:
			album_info = self.db.albums.find({"_id": album_id})[0]
			for tag in album_info["tag"]:
				self.remove_album_from_tag(album_id, tag)
			self.db.albums.remove({"_id": album_id})
	...
	def remove_user(self, user):
		...
		for image in image_list:
			...
			self.db.remove({"_id": image["_id"]})
		album_list = self.get_albums_by_user(user)
		for album in album_list:
			self.db.remove({"_id": album["_id"]})

	def get_image_by_id(self, id):
		result = self.db.images.find({"_id": id})[0]
		try:
	...
	def get_images_by_album(self, album):
		result = []
		result_temp = self.db.albums.find({"_id": album}, {"_id": 0, "image_list": 1})[0]
	...
	def get_album_by_id(self, id):
		result = self.db.albums.find({"_id": id})[0]
		return result
